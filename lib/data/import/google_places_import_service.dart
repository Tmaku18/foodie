...
class GooglePlacesImportService {
  ...
  Future<List<ImportedRestaurantRecord>> fetchNearbyRestaurants({
    ...
    if (apiKey.isEmpty) {
      throw StateError(
        'Missing GOOGLE_MAPS_API_KEY. Pass --dart-define=GOOGLE_MAPS_API_KEY=your_key',
      );
    }
    ...
    final response = await _client.post(
      ...
      headers: {
        ...
        'X-Goog-FieldMask':
            'places.id,places.displayName,places.types,places.rating,places.location,places.formattedAddress,places.photos',
      },
      body: requestBody,
    );
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) {
      throw StateError(
        'Google Places request failed (${response.statusCode}): ${response.body}',
      );
    }
  ...
  ImportedRestaurantRecord _mapPlaceToRecord(Map<String, dynamic> place) {
    ...
    final lat =
        ...
        PlacesImportConfig.gsuStudentCenterEastLat;
    final lng =
        (location['longitude'] as num?)?.toDouble() ??
        PlacesImportConfig.gsuStudentCenterEastLng;
    ...
    return ImportedRestaurantRecord(
      ...
      photoNames: (place['photos'] as List<dynamic>? ?? const <dynamic>[])
          .whereType<Map<String, dynamic>>()
          .map((photo) => (photo['name'] as String?)?.trim() ?? '')
